...
async function handleVictory(character, enemy, res, combatLog) {
  ...
  if (scroll) {
    const classId = scroll.id.replace('scroll_', '').replace(/_/g, '');
    const mappedClassId = { scrollflameblade: 'flameblade', scrollshadowdancer: 'shadowDancer', scrollstormranger: 'stormRanger', scrollfrostweaver: 'frostWeaver' }[classId] || classId;
    try {
