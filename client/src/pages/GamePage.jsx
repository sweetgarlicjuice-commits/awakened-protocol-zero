        ...
        <main className="flex-1 flex flex-col">
          ...
          <div className="flex-1 p-4 md:p-6 overflow-auto">
            ...
            {activeTab === 'tower' && (
              <div className="max-w-4xl mx-auto">
                <div className="bg-void-800/50 rounded-xl p-8 neon-border text-center">
                  ...
                  <h3 className="font-display text-2xl text-purple-400 mb-2">TOWER SYSTEM</h3>
                  <p className="text-gray-500 mb-6">Coming in Phase 2</p>
                  <div className="inline-flex items-center gap-2 px-4 py-2 bg-purple-500/10 rounded-full text-purple-400 text-sm">
            ...
            {activeTab === 'inventory' && (
              <div className="max-w-4xl mx-auto">
                <div className="bg-void-800/50 rounded-xl p-6 neon-border">
                  <h3 className="font-display text-lg text-purple-400 mb-4">INVENTORY ({character.inventory?.length || 0} / {character.inventorySize})</h3>
                  {(!character.inventory || character.inventory.length === 0) ? (
                    <div className="text-center py-12 text-gray-500">
                      <div className="text-4xl mb-3">🎒</div>
                      <p>Your inventory is empty</p>
                      <p className="text-sm">Explore towers to find items!</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-5 md:grid-cols-8 lg:grid-cols-10 gap-2">
                      {character.inventory.map((item, index) => (
                        <div key={index} className={`aspect-square bg-void-900 rounded-lg border p-2 flex items-center justify-center cursor-pointer hover:scale-105 transition-transform rarity-${item.rarity}`} title={item.name}>
